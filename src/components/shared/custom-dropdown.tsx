...
export function CustomDropdownItem({ children, onClick, className }: { children: React.ReactNode, onClick?: () => void, className?: string }) {
  return (
    <div 
      onClick={onClick}
      className={cn("block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-slate-900 cursor-pointer w-full text-left", className)}
    >
      {children}
    </div>
  );
...
export function CustomDropdownLabel({ children }: { children: React.ReactNode }) {
  return <div className="px-4 py-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">{children}</div>;
}
