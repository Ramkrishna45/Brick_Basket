  ...
  if (!project) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="text-slate-500 mt-2 text-center max-w-sm">The project you are looking for does not exist or you don't have access.</p>
      </div>
    );
  }
            ...
            <div className="md:col-span-1 space-y-6">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-medium text-slate-900">{project.clientName || project.customer?.name || "N/A"}</div>
                      <div className="text-xs text-slate-500">Client / Owner</div>
                    </div>
                  </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-3 text-sm">
                  <div className="flex justify-between border-b pb-2"><span className="text-slate-500">Plot Size</span><span className="font-medium">{project.plotSize || "N/A"}</span></div>
                  <div className="flex justify-between border-b pb-2"><span className="text-slate-500">Built-up Area</span><span className="font-medium">{project.builtUpArea || "N/A"}</span></div>
                  <div className="flex justify-between"><span className="text-slate-500">Plan Name</span><span className="font-medium">{project.planName || "Custom Plan"}</span></div>
                </CardContent>
              </Card>
            </div>
                  ...
                  <div className="relative">
                    ...
                    <div className="space-y-3">
                          ...
                          <motion.div key={milestone.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="relative flex gap-4 z-10">
                            ...
                            <div className="flex-shrink-0 flex items-start pt-2">
                              {milestone.status === "completed" ? (
                                ...
                                <div className="h-10 w-10 rounded-full bg-white border-2 border-slate-300 flex items-center justify-center">
                            ...
                            {/* Content */}
                            <div className={cn("flex-1 pb-3", i < project.milestones.length - 1 ? "border-b border-slate-100" : "")}>
                              <button onClick={() => setExpanded(isExpanded ? null : milestone.id)} className="flex items-center justify-between w-full text-left py-1.5 focus:outline-none">
                                <div>
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <span className={cn("font-semibold text-sm", milestone.status === "completed" ? "text-slate-700" : milestone.status === "in_progress" ? "text-amber-700" : "text-slate-400")}>
                                      {milestone.title}
                                    </span>
                                    {milestone.status === "in_progress" && <Badge className="bg-amber-50 text-amber-700 border-amber-200 text-xs py-0">Active</Badge>}
                                  </div>
                              ...
                              {isExpanded && milestone.notes && (
                                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="overflow-hidden">
                                  <div className="bg-amber-50 border border-amber-100 rounded-lg p-3 mt-1">
                                    <div className="text-xs font-medium text-amber-800 mb-1">Notes</div>
                                    <p className="text-xs text-amber-900 leading-relaxed">{milestone.notes}</p>
                                  </div>
                                </motion.div>
                              )}
                            </div>
                      ...
                      {project.milestones?.length === 0 && (
                         <div className="text-sm text-slate-500 pl-14 py-4">No construction milestones created yet.</div>
                      )}
                    </div>
                  </div>
                </CardContent>
            ...
            <CardContent>
              {(!project.progressUpdates || project.progressUpdates.length === 0) ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-slate-500 font-medium">No progress updates available.</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {project.progressUpdates.map((update: any) => (
                    <div key={update.id} className="border-b border-slate-100 pb-6 last:border-0 last:pb-0">
            ...
            <CardContent>
              {(!project.payments || project.payments.length === 0) ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-slate-500 font-medium">No payments scheduled yet.</p>
                </div>
              ) : (
                  ...
                  <table className="w-full text-sm text-left">
                    <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b">
                      <tr>
                        ...
                        <th className="px-4 py-3 rounded-tr-lg text-right">Paid On</th>
                      </tr>
                    </thead>
                    <tbody>
                      {project.payments.map((payment: any, i: number) => (
                        <tr key={payment.id} className="border-b last:border-0 hover:bg-slate-50">
                          <td className="px-4 py-3 font-medium text-slate-900">{payment.name}</td>
                          <td className="px-4 py-3 text-slate-500">{payment.dueDate ? format(new Date(payment.dueDate), "MMM d, yyyy") : "TBD"}</td>
                          <td className="px-4 py-3 font-semibold text-slate-900">₹{payment.amount.toLocaleString("en-IN")}</td>
                          <td className="px-4 py-3">
                            <Badge className={cn(
                              "text-[10px]",
                              payment.status === "paid" ? "bg-emerald-100 text-emerald-700 hover:bg-emerald-100" 
