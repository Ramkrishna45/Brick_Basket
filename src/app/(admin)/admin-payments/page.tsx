...
export default function AdminPaymentsPage() {
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Payment Tracking</h1>

      {/* Summary */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total Revenue", value: formatINR(totalRevenue), icon: IndianRupee, color: "text-slate-700 dark:text-slate-300", bg: "bg-slate-50 dark:bg-slate-900" },
          { label: "Collected", value: formatINR(totalCollected), icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-50" },
          { label: "Pending", value: formatINR(totalPending), icon: TrendingDown, color: "text-amber-600 dark:text-amber-500", bg: "bg-amber-50 dark:bg-amber-950/40" },
                ...
                {projects.map((p, i) => {
                  ...
                  return (
                    <motion.tr key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.06 }}
                      ...
                      <TableCell className="font-medium text-slate-900 dark:text-slate-100 text-sm">{p.customer?.name || "Unassigned"}</TableCell>
                      <TableCell className="text-sm text-slate-600 dark:text-slate-400">{p.name}</TableCell>
                      <TableCell className="font-semibold text-slate-900 dark:text-slate-100 text-sm">{formatINR(p.totalValue)}</TableCell>
                      <TableCell className="text-emerald-600 font-medium text-sm">{formatINR(p.amountPaid)}</TableCell>
                      <TableCell className="text-amber-600 dark:text-amber-500 font-medium text-sm">{formatINR(remaining)}</TableCell>
                      <TableCell>
                        <div className="w-24">
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          <Button render={<Link href={`/admin-payments/${p.id}`} />} variant="ghost" size="sm" className="h-7 text-xs hover:text-amber-600 dark:text-amber-500">
                            Details
                          </Button>
                          <Button render={<Link href={`/admin-payments/${p.id}`} />} size="sm" className="h-7 text-xs bg-emerald-600 hover:bg-emerald-700">
                            Manage
                          </Button>
                        </div>
                      </TableCell>
              ...
              return (
                <div key={p.id} className="p-4 space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="font-medium text-slate-900 dark:text-slate-100">{p.customer?.name || "Unassigned"}</div>
