...
import { CheckCircle2, ArrowRight, ArrowLeft, Phone, Mail, MessageSquare, Sparkles } from "lucide-react";
...
export default function DashboardEnquiryPage() {
  ...
  useEffect(() => {
    if (user) {
      if (user.name) form1.setValue("name", user.name);
      if (user.email) form1.setValue("email", user.email);
      if (user.phone) form1.setValue("phone", user.phone);
    }
  }, [user, form1]);
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        >
          ...
          <div className="mt-6">
            <Button onClick={() => router.push("/dashboard")} className="bg-amber-600 hover:bg-amber-700 text-white gap-2">
